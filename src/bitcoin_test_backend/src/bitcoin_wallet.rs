...
use crate::{bitcoin_api::{self, JsonOutPoint}, ecdsa_api, types::ECDSAPublicKeyReply};
use bitcoin::{
    absolute::LockTime, bech32, blockdata::{script::Builder, witness::Witness}, consensus::serialize, hashes::Hash, sighash::{EcdsaSighashType, SegwitV0Sighash, SighashCache}, Address, AddressType, Network, OutPoint, Script, ScriptBuf, Transaction, TxIn, TxOut, Txid
};
...
//     let own_address = account_to_p2wpkh_address(network, key_name.clone(),vec![own_public_key]).await;

//     print("Fetching UTXOs...");
//     // Note that pagination may have to be used to get all UTXOs for the given address.
//     // For the sake of simplicity, it is assumed here that the `utxo` field in the response
//     // contains all UTXOs.
//     let own_utxos = bitcoin_api::get_utxo(network, own_address.clone())
//         .await;
...
//     );
//     let sighashcache = SighashCache::new(&transaction);
//     let txclone = transaction.clone();
//     for (index, input) in transaction.input.iter_mut().enumerate() {
//         // let sighash =
//         //     txclone.signature_hash(index, &own_address.script_pubkey(), SIG_HASH_TYPE.to_u32());
//         let signhash = sighashcache.segwit_signature_hash(index, &own_address.script_pubkey(), input., EcdsaSighashType::All).expect("build sighash failed");
